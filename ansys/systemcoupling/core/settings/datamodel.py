...
def to_python_name(syc_name: str) -> str:
    ...
    if not syc_name:
        return syc_name
    name = syc_name#.translate(_ttable)
    ...
    def to_python_keys(cls, value):
        ...
        if isinstance(value, collections.abc.Mapping):
            ...
            for mname in cls.child_names:
                ...
                mvalue = value.get(ccls.syc_name, undef)
...
class ListObject(SettingsBase[ListStateType]):
    ...
    def to_syc_keys(cls, value):
        ...
        if isinstance(value, collections.abc.Sequence):
            return [cls.child_object_type.to_syc_keys(v) for v in value]
        else:
            return value
    ...
    def get_size(self) -> int:
        ...
        int
        """
        return self.sycproxy.get_list_size(self.path)
    ...
    def resize(self, size: int):
        ...
        size: int
              New size
        """
        self.sycproxy.resize_list_object(self.path, size)
...
class Command(Base):
    ...
    def __call__(self, **kwds):
        ...
        return self.sycproxy.execute_cmd(self._parent.path,
                self.obj_name,
                **newkwds)
...
def get_cls(name, info, parent = None):
    ...
    try:
        ...
        if children or parameters:
            ...
            if parameters:
                for cname, cinfo in parameters.items():
                    ccls = get_cls(cname, cinfo, cls)
                    #pylint: disable=no-member
                    cls.child_names.append(ccls.__name__)
                    setattr(cls, ccls.__name__, ccls)
        ...
        if object_type:
            cls.child_object_type = \
                    get_cls('child_object_type', info, cls)
    except Exception:
...
def get_root(sycproxy) -> Group:
    ...
    try:
        ...
        if datamodel222.SHASH != _gethash(obj_info):
