...
fluent_session = pyfluent.launch_fluent(product_version="24.1.0",start_transcript=False)
...
# Change the "maxium heat output" settings
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_2'].real_value = 10.0
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_2'].display_name = "Maxium Heat Output"
# %%
# Change the "target temperature" settings
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_3'].real_value = 350
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_3'].display_name = 'Target_Temperature'

# %%
# Change the "heat scale factor" settings
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_4'].real_value = 2.0
setup.coupling_participant[fmu_part_name].fmu_parameter['Real_4'].display_name = 'Heat_Scale_Factor'
...
sensorInterface = setup.add_interface(side_one_participant = fluent_part_name, 
                                     side_one_regions = ['sensor'],
                                     side_two_participant = fmu_part_name)

# Create a coupling interface for FMU -> Fluent (FMU to heat source)
heatSourceInterface = setup.add_interface(side_one_participant = fmu_part_name,
                                         side_two_participant = fluent_part_name,
                                         side_two_regions = ['heat_source'])

# Create data transfer for "temperature"
temperatureDataTransfer = setup.add_data_transfer(interface = sensorInterface,
                                                 target_side = "Two",
                                                 source_variable = "temperature",
                                                 target_variable = "Real_0")

# Create data transfer for "heatflow"
heatFlowDataTransfer = setup.add_data_transfer(interface = heatSourceInterface,
                                              target_side = "Two",
...
fluent_session.results.surfaces.plane_surface[plane_name].method = normal_plane + "-plane"
...
for i in range(0, number_of_planes):
    ...
    fluent_session.results.surfaces.plane_surface[multiplane_name].method = normal_multiplane + "-plane"
    fluent_session.results.surfaces.plane_surface[multiplane_name].z = float(i) * step + step
...
def contour(fluent_session, surface_list, surface_name, field, color):
        ...
        fluent_session.results.graphics.contour[contour_name] = {
            ...
            "range_option": {"option": "auto-range-on", "auto_range_on": {"global_range": True}},
...
def scene(fluent_session, object_name):
    ...
    fluent_session.results.scene[scene_name].graphics_objects["outline"].transparency = 90
...
contour(fluent_session, surface_list=["plane-zx-0.0"] , surface_name="plane-zx-0.0"  , field="total-temperature"  , color="field-temperature")
contour(fluent_session, surface_list=["plane-zx-0.0"] , surface_name="plane-zx-0.0"  , field="velocity-magnitude" , color="field-velocity")
contour(fluent_session, surface_list=mutliplane_list  , surface_name="multiplane-xy" , field="total-temperature"  , color="field-temperature")
contour(fluent_session, surface_list=mutliplane_list  , surface_name="multiplane-xy" , field="velocity-magnitude" , color="field-velocity")
vector(fluent_session , surfaces_list=["plane-zx-0.0"], surface_name="plane-zx-0.0"  , field="total-temperature")
